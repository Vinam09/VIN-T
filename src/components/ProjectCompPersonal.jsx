...
function projectCompPersonal({picture,name,hyperlink,target,gitlink,skills,description}) {
    return (
        ...
        <div className = "xl:w-5/12 md:w-1/2 sm:w-full p-4 flex flex-col justify-between">
            <div>
                <a href={hyperlink} rel="noreferrer" target={target}><h2 className="text-primary 2xl:text-2xl md:text-xl text-left hover:text-hovercolor">{name}</h2></a>
            ...
            <div className="flex flex-row">
                <a href={hyperlink} rel="noreferrer" target={target}><img src ={link} className="mt-4 w-6 pb-2 mr-4 hover:animate-bounce"/></a>
                <a href={gitlink} rel="noreferrer" target="_blank"><img src ={github} className="mt-4 w-6 pb-2 hover:animate-bounce"/></a>
            </div>
        </div>
    </div>
    )
